...
public class DrawAgent {
    ...
    public static final int FONT_CHAR_SIZE = 16;
    ...
    public void drawChars(Canvas c, char[] chrs , Bitmap font, PointF location, int offset, int length)
    {
        int l = Math.min(offset + length, chrs.length);
        for(int i=offset; i<l; i++) {
            int ch = (int)(chrs[i]);
            if(ch < 32 || ch > 127) {
                ch = 127;
            }
            src.left = (ch % 16) * FONT_CHAR_SIZE;
            src.top = ((ch - 32) / 16) * FONT_CHAR_SIZE;
            src.right = src.left + FONT_CHAR_SIZE;
            src.bottom = src.top + FONT_CHAR_SIZE;
            drawSprite(c, font, src, location);
            location.x += (float)FONT_CHAR_SIZE;
        }
    }
